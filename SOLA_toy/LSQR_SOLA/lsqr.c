...
void lsqr_error(char *msg, int code)
{
    fprintf(stderr, "\t%s\n", msg);
    exit(code);
}
...
lvec *alloc_lvec(long lvec_size)
{
    ...
    if (!lng_vec)
        lsqr_error
            ("lsqr: vector allocation failure in function alloc_lvec()",
             -1);

    lng_vec->elements = (long *) malloc((lvec_size) * sizeof(long));
    if (!lng_vec->elements)
        lsqr_error
            ("lsqr: element allocation failure in function alloc_lvec()",
             -1);
...
dvec *alloc_dvec(long dvec_size)
{
    ...
    if (!dbl_vec)
        lsqr_error
            ("lsqr: vector allocation failure in function alloc_dvec()",
             -1);

    dbl_vec->elements = (double *) malloc((dvec_size) * sizeof(double));
    if (!dbl_vec->elements) {
        fprintf(stderr, "dvec_size = %ld\n", dvec_size);
        lsqr_error
            ("lsqr: element allocation failure in function alloc_dvec()",
             -1);
    }
...
void alloc_lsqr_mem(lsqr_input ** in_struct,
                    ...
                    long max_num_rows, long max_num_cols)
{
    *in_struct = (lsqr_input *) alloc_lsqr_in(max_num_rows, max_num_cols);
    if (!in_struct)
        lsqr_error
            ("lsqr: input structure allocation failure in function alloc_lsqr_in()",
             -1);

    *out_struct =
        (lsqr_output *) alloc_lsqr_out(max_num_rows, max_num_cols);
    if (!out_struct)
        lsqr_error
            ("lsqr: output structure allocation failure in function alloc_lsqr_out()",
             -1);
    ...
    *wrk_struct = (lsqr_work *) alloc_lsqr_wrk(max_num_rows, max_num_cols);
    if (!wrk_struct)
        lsqr_error
            ("lsqr: work structure allocation failure in function alloc_lsqr_wrk()",
             -1);

    *fnc_struct = (lsqr_func *) alloc_lsqr_fnc();
    if (!fnc_struct)
        lsqr_error
            ("lsqr: work structure allocation failure in function alloc_lsqr_fnc()",
             -1);
...
void free_lsqr_mem(lsqr_input * in_struct,
                   lsqr_output * out_struct,
                   lsqr_work * wrk_struct, lsqr_func * fnc_struct)
{
...
lsqr_input *alloc_lsqr_in(long max_num_rows, long max_num_cols)
{
    ...
    if (!in_struct->rhs_vec)
        lsqr_error
            ("lsqr: right hand side vector \'b\' allocation failure in function alloc_lsqr_in()",
             -1);

    in_struct->sol_vec = (dvec *) alloc_dvec(max_num_cols);
    if (!in_struct->sol_vec)
        lsqr_error
            ("lsqr: solution vector \'x\' allocation failure in function alloc_lsqr_in()",
             -1);
...
lsqr_output *alloc_lsqr_out(long max_num_rows, long max_num_cols)
{
    lsqr_output *out_struct;
    ...
    if (!out_struct->std_err_vec)
        lsqr_error
            ("lsqr: standard error vector \'e\' allocation failure in function alloc_lsqr_out()",
             -1);
...
lsqr_work *alloc_lsqr_wrk(long max_num_rows, long max_num_cols)
{
    lsqr_work *wrk_struct;
    ...
    if (!wrk_struct->bidiag_wrk_vec)
        lsqr_error
            ("lsqr: bidiagonalization work vector \'v\' allocation failure in function alloc_lsqr_wrk()",
             -1);

    wrk_struct->srch_dir_vec = (dvec *) alloc_dvec(max_num_cols);
    if (!wrk_struct->srch_dir_vec)
        lsqr_error
            ("lsqr: search direction vector \'w\' allocation failure in function alloc_lsqr_wrk()",
             -1);
...
lsqr_func *alloc_lsqr_fnc()
{
    ...
    if (!fnc_struct)
        lsqr_error
            ("lsqr: function structure allocation failure in function alloc_lsqr_fnc()",
             -1);
...
void lsqr(lsqr_input * input, lsqr_output * output, lsqr_work * work,
          lsqr_func * func, void *prod)
{
    double dvec_norm2(dvec *);

    long indx,
        /*num_iter, */
     term_iter, term_iter_max;
    ...
    double alpha,
        ...
        resid_tol,
        cond_tol,
        resid_tol_mach, temp, stop_crit_1, stop_crit_2, stop_crit_3;

    static char term_msg[9][80] = {
	...
	BTOL = %10.2e\t\tITERLIM = %10li\n\n", input->num_rows, input->num_cols, input->damp_val, input->rel_mat_err, input->cond_lim, input->rel_rhs_err, input->max_iter);
...
*/
	...
	|| b ||_2 = %13.5e\t|| x - x0 ||_2 = %13.5e\n\n", output->term_flag, output->num_iters, output->frob_mat_norm, output->mat_cond_num, output->resid_norm, output->mat_resid_norm, bnorm, output->sol_norm);
...
*/
    if (input->lsqr_fp_out != NULL) {
        fprintf(input->lsqr_fp_out, "  ITER     || r ||    Compatible  \
||A^T r|| / ||A|| ||r||  || A ||    cond( A )\n\n");
        ...
        fprintf(input->lsqr_fp_out,
                ...
                stop_crit_2, output->frob_mat_norm, output->mat_cond_num);
    }
...
*/
   ...
   // printf("\n[ - C - ]\n");
    ...
    //printf("\n[ - D - ]\n");
            func->mat_vec_prod(1, work->bidiag_wrk_vec, input->rhs_vec,
                               prod);
...
*/
        for (indx = 0; indx < input->num_cols; indx++) {
             ...
             */
            ddnorm +=
                sqr((1.0 / rho) * work->srch_dir_vec->elements[indx]);
...
*/
        ...
        if (please_dump_lsqr && !(output->num_iters % please_dump_lsqr)) {
            char filename[256];
            sprintf(filename, "iter-%ld.txt", output->num_iters);
            // write_vector((struct vector_t *) output->sol_vec, filename); // ZAROLI -- COMMENTED -- WHY?
        }
